                ...
                if "/" in split:
                    ...
                    for j, instr in enumerate(item['instructions']):
                        try:
                            ...
                            ''' BERT tokenizer '''
                            instr_tokens = tokenizer.tokenize(instr)    # 将 instr 按照字母拆解成 token
                            padded_instr_tokens, num_words = pad_instr_tokens(instr_tokens, args.maxInput)      # 限制 token 长度 (maxInput=78个单词之后的信息将被舍弃), 并添加 [cls], [sep] 字段
                            new_item['instr_encoding'] = tokenizer.convert_tokens_to_ids(padded_instr_tokens)   # 将 token 转为数字编码
